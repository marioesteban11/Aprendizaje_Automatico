...
net_der = patternnet(5, "trainlm") 
net_der.trainParam.goal = 0.05;
net_der.trainParam.min_grad = 0;
net_der.trainParam.max_fail = 25;
net_der.trainParam.epochs = 1000;
net_der = train(net_der,datos_bola_ia, datos_valor );
...
while(time <= 1000)
    time = time+1;
    p_valor = sim(net_der,[posy_ball + (size_ball / 2); posinity_player + (height_player / 2); speed_ballx; speed_bally]);
